    ...
    .map((message) => {
      ...
      return `
        <article class="chat__message${isOwnMessage ? " chat__message--own" : ""}">
          <div class="chat__message__avatar">${initials(message.authorName)}</div>

          <div class="chat__message__content">
            <p class="chat__message__content__meta">
              <strong>${message.authorName}</strong> · ${message.createdAt}
            </p>
  ...
  return `
    <div class="ticketAside__section ticketAside__section--statusControl">
      ...
      <div class="ticketStatusControl">
        <select
          class="ticketStatusControl__select"
        ...
        <button
          class="primaryButton ticketStatusControl__button"
          id="updateTicketStatusButton"
          type="button"
        >
          Zapisz status
        </button>
      ...
      <p
        class="ticketStatusControl__message"
...
const renderResolutionConfirmation = (ticket, isTechnician) => {
  if (isTechnician || ticket.status !== "Zakończone") {
    return "";
  }
  ...
  return `
    <div class="ticketAside__section ticketAside__section--resolution">
      ...
      <div class="resolutionConfirmation">
        <button
          class="primaryButton resolutionConfirmation__button"
          id="acceptResolutionButton"
          type="button"
        >
          Tak
        </button>
        ...
        <button
          class="secondaryButton resolutionConfirmation__button"
          id="rejectResolutionButton"
          type="button"
        >
          Nie
        </button>
      ...
      <p
        class="ticketStatusControl__message"
        id="resolutionConfirmationMessage"
        aria-live="polite"
      ></p>
    </div>
  `;
...
export const initializeTicketDetailsPage = () => {
  ...
  // Technik odczytuje wiadomości dopiero po przypisaniu zgłoszenia do siebie.
  ...
  const assignment = canAssign
    ? `
      <button class="primaryButton" id="assignTicketButton" type="button">
    ...
    : isTechnician
      ? `
        <span class="assignmentInfo">
          ${
            isAssignedToMe
              ? "Zgłoszenie przypisane do Ciebie"
              : "Zgłoszenie jest już przypisane do innego technika"
          }
        </span>
      `
      : "";
  ...
  const reply = canReply
    ? `
      <form class="chat__reply" id="replyForm">
        <textarea
          class="chat__reply__textarea"
          id="replyTextarea"
          rows="1"
    ...
    : `
      <div class="chat__reply">
        <span class="assignmentInfo">
          Odpowiedzi może dodawać technik przypisany do zgłoszenia.
        </span>
      </div>
    `;
    ...
    <section class="ticketLayout">
          ...
          <div class="ticketHeader__upper">
            <div>
              <div class="ticketHeader__upper__meta">
                ...
                <span class="priorityBadge priorityBadge--${TicketTable.getPriorityClass(ticket.priority)}">
                  ${ticket.priority}
                </span>

                <span class="statusBadge statusBadge--${TicketTable.getStatusClass(displayStatus)}">
                  ${displayStatus}
                </span>
              </div>
      ...
      <aside class="ticketAside">
        ...
        <div class="ticketAside__section">
          ...
          <p class="ticketAside__section__value">
        ...
        <div class="ticketAside__section">
          <div class="ticketAside__section__grid">
            <div>
              <p class="ticketAside__section__label">Kategoria</p>
              <p>${ticket.category}</p>
            </div>

            <div>
              <p class="ticketAside__section__label">Priorytet</p>
              <p>${ticket.priority}</p>
            </div>

            <div>
              <p class="ticketAside__section__label">Utworzono</p>
              <p>${ticket.createdAt}</p>
            </div>

            <div>
              <p class="ticketAside__section__label">Status</p>
              <p id="ticketAsideStatus">${displayStatus}</p>
            </div>
          </div>
        </div>
  ...
  document
    ...
    ?.addEventListener("click", () => {
      if (TicketRepository.assignTo(ticket.id, email)) {
  ...
  document
    ...
    ?.addEventListener("click", () => {
      ...
      if (statusMessage) {
        statusMessage.textContent = "Status został zapisany.";
      }
  ...
  document
    ...
    ?.addEventListener("click", () => {
      ...
      if (!TicketRepository.rejectResolution(ticket.id, email)) {
        if (message) {
          message.textContent = "Nie udało się wznowić zgłoszenia.";
        }
